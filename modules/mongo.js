...
const url = process.env.MONGO_URL_159;
console.log("process.env.MONGO_URL_159",process.env.MONGO_URL_159);
const options = {
	//bufferMaxEntries: 2,
	keepAlive: true,
...
const db = (() => {
	...
	GLOBAL_DBREL = null;
	GLOBAL_DBREL_ATENDE = null;

	self.isConnected = () => {
		return (
			!!GLOBAL_DBREL && !!GLOBAL_DBREL.topology && GLOBAL_DBREL.topology.isConnected()
		);
	};
	...
	self.connect = async () => {
		return new Promise(async (resolve, reject) => {
			...
			let dd = await MongoClient.connect(url, options, async (err, db) => {
				...
				GLOBAL_DBREL = db;
				GLOBAL_DBREL_ATENDE = await db.db("atende");
				resolve(); //dbo);
			});
		});
	...
	self.infos = (database, collection) => {
		try {
			return new Promise(async (resolve, reject) => {
				//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : await GLOBAL_DBREL.db(database)
				await GLOBAL_DBREL.admin();
				let opt = await GLOBAL_DBREL.admin().serverStatus({ name: collection });
		...
		try {
			...
			return new Promise(async (resolve, reject) => {
				try {
					//database.dbo
					//dbo
					const dbase = GLOBAL_DBREL;
					/*
            GLOBAL_DBREL_ATENDE.s.databaseName == database
              ? GLOBAL_DBREL_ATENDE
              : await GLOBAL_DBREL.db(database); */
					dbase
					//GLOBAL_DBREL_ATENDE
						.db(database)
	...
	self.findById = (database, collection, id) => {
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					//database.dbo
					// const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					//dbase
					GLOBAL_DBREL_ATENDE.collection(collection).findOne(
						{ _id: ObjectId(id) },
						function (err, data) {
							if (err) reject(err);
							resolve(data);
	...
	self.find = (
		...
		try {
			return new Promise(async (resolve, reject) => {
				try {
					const dbase =
            GLOBAL_DBREL_ATENDE.s.databaseName == database ? GLOBAL_DBREL_ATENDE : await GLOBAL_DBREL.db(database);
		...
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					//dbase
					GLOBAL_DBREL_ATENDE.collection(collection).deleteOne(
						where,
						(err, data) => {
							if (err) reject(err);
							resolve(data);
		...
		try {
			return new Promise((resolve, reject) => {
				try {
					...
					//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					// dbase
					GLOBAL_DBREL_ATENDE.collection(collection).deleteMany(
						where,
						(err, data) => {
							if (err) reject(err);
							resolve(data);
		...
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					// dbase
					GLOBAL_DBREL_ATENDE.collection(collection).insertMany(
						insertData,
						(err, data) => {
							if (err) reject(err);
							// self.closeDb();
		...
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					// const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					// dbase
					GLOBAL_DBREL_ATENDE.collection(collection).insertOne(
						data,
						{ w: 1, j: true },
						(err, result) => {
	...
	self.updateOne = (database, collection, where, set, arrayFilters = {}, unset = {}) => {
		try {
			if (!isObject(where)) {
				self.throwErr("Where deve ser um objeto");
			}
		...
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					//dbase
					GLOBAL_DBREL_ATENDE.collection(collection).updateMany(
						where,
						Aset,
						(err, data) => {
							if (err) reject(err);
		...
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					// const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					//dbase
					GLOBAL_DBREL_ATENDE.collection(collection).updateOne(
						{ _id: ObjectId(id) },
						{ $set: set },
						(err, data) => {
							if (err) reject(err);
	...
	self.aggregate = (database, collection, params) => {
		try {
			//console.log("  agreggate --  ",GLOBAL_DBREL)
			...
			return new Promise((resolve, reject) => {
				try {
					...
					//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					//dbase
					GLOBAL_DBREL_ATENDE.collection(collection)
						.aggregate(params)
						.toArray((err, data) => {
							if (err) reject(err);
							resolve(data);
		...
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					//dbase
					GLOBAL_DBREL_ATENDE.collection(collection).distinct(
						field,
						where,
						(err, data) => {
							if (err) reject(err);
		...
		try {
			...
			return new Promise((resolve, reject) => {
				try {
					...
					//const dbase = GLOBAL_DBREL_ATENDE.s.databaseName==database ? GLOBAL_DBREL_ATENDE : GLOBAL_DBREL.db(database)
					//dbase
					GLOBAL_DBREL_ATENDE.collection(collection).count(where, (err, data) => {
						if (err) reject(err);
						// self.closeDb();
						resolve({ count: data });
					});
