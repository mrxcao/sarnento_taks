...
const chVinculadas = async (data, unidade) => {
	try {
					...
					pipeline: [
						...
						{ $unwind: "$interacoes" },
						{
							$match: {
								...
								"interacoes.origem.usuario.unidade": unidade
							}
						},
					...
					pipeline: [
						...
						{ $unwind: "$interacoes" },
						{
							$match: {
								...
								"interacoes.usuario.unidade": unidade
							}
						},
					...
					pipeline: [
						...
						{ $unwind: "$interacoes.unidadeDestino" },
						{
							$match: {
								...
								"interacoes.unidadeDestino.unidade": unidade
							}
						},
			...
			//=== concluidos autom ===
			{
				$lookup: {
					...
					pipeline: [
						...
						{ $match: { "atendente.unidade.unidade": unidade } },
					...
					pipeline: [
						{
							$match: {
								...
								"historicos.unidade.unidade.unidade": unidade
							}
						},
						...
						{ $match: { "unidade.unidade.unidade": unidade } },
						//{$sort: {id:-1}}
						//{$count:"qtd"}
					...
					pipeline: [
						...
						{ $unwind: "$interacoes.stats" },
						{
							$match: {
								...
								"interacoes.stats.unidade.unidade": unidade
							}
						},
			...
			//=== pendentes ===
			{
				$lookup: {
					...
					pipeline: [
						{
							$match: {
								data: { $gte: new Date("2020-01-01") }, exportacaoGEOTR: { $exists: false }, "categoria._id": { $ne: ObjectId("5d9359fcdc35bc02f873b513") },
